...
def get_links_from_page(html):#получение ссылок из всех объявлений на странице
    ...
    f = open('someparsingresults/YoulaLinks.txt', 'a')
    for ad in ads:
        try:
...
def main():
    #https://auto.youla.ru/chel/search/?brandId=local&brandOrigin=1&sellers=1&searchOrder=3&photo=1&page=2
    useragents = open('hideparcing/useragents.txt').read().split('\n')
    proxies = open('hideparcing/proxies.txt').read().split('\n')

    base_url = 'https://auto.youla.ru/chel/search/?'
    page_part = 'page='
    query_part ='brandId=local&brandOrigin=1&sellers=1&searchOrder=3&photo=1&'

    url_gen = base_url + query_part
    useragent = {'User-Agent': choice(useragents)}
    proxy = {'http': 'http://' + choice(proxies)}
    total_pages = get_total_pages(get_html('https://auto.youla.ru/chel/search/?kladdr%5B0%5D=2644&kladdr%5B1%5D=66&brandId=local&brandOrigin=1&sellers=1&searchOrder=3&photo=1&page=2', useragent, proxy))
